...
**Answer** Creating a separate variable allows you to look at the columns and rows you specifically selected/filtered without overriding the original dataset, so if you make a mistake, you can go back to the original dataset and try again.
...
**Answer** The OR operator gives us 41,011 records, which is significantly higher than AND because it takes into account both Theatre classes as well as ll other departments.
...
**Answer:** The expenditure_date column is now in a yyyy-mm-dd format. It changed the order of the original from the mdy format and also made the month two digits. There are also dashes between the year, month and day instead of slashes.
...
**Answer** It added all the expenitures that happened in a particular month for each year. The date format stayed the same, but they just used the first day of the month every time and changed the month and year accordingly.
...
**Answer** It created a new column for the percentage of votes that went to Wes Moore by dividing the number of the votes Moore got with the total, which we created by adding together the votes for all the other candidates.
...
**Answer** It arranged the counties in order of highest percentage of Moore voters to lowest.
...
**Answer** I think the counties with the higher percentages are more compelling because they give a better idea of why Moore won the election.
...
**Answer** The function "str_to_lower" makes all the letters in the character column lowercase, and the "str_to_title" function capitalizes only the first letter of every word.
...
**Answer** It is pulling the specific name of the state in the address column and then creating a new column with just the state's abbreviation. 
...
**Answer** Yes, I think it's adding all the expenditures for each state that was listed in the case-when function, with NA representing all the other states that weren't specified, which explains why it's so high up on the list.
...
1. Which instructors teach the most classes (same question from previous prelab but this time taking into account the people being listed as co-instructors in a list)?

2.  Which department has the most classes focus that on the topics of race/diversity?

3.  How many classes have prerequisites?
