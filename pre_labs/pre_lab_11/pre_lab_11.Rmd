...
**Task** Run the following code and describe the dataframe it outputs. **Answer** It has 99 press releases with the date, title, url and text of each press release.
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** It removes the words that are being used more than once, but it's case-sensitive so it keeps the same word that has different cases.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It shows all the unique words and tokens found in the press releases.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** It's a bar chart showing how many times each word shows up in the press releases. It's not interesting because they're mostly articles.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** The code shows the frequency and the percentage of the the words not including the stop words, which makes the results more interesting. You see more words like health, baltimore, etc. 
...
**Task** Run the following code and describe the results. **Answer** You can see the most frequently-used words for each year. There are some words that were used more in some years than others, such as COVID.
...
**Task** Run the following code and describe the results. **Answer** This code shows which two words show up together often. Most of the results are pretty useful and make sense, but some, like "19 pandemic" isn't as accurate. 
...
**Task** Run the following code and describe the results. **Answer** This code does the same thing as the previous task but for 2022. It looks like PG County was a big topic in 2022 as it comes up twice.
...
**Task** Run the following code and describe the results. **Answer** 2023 has a lot about human and equal rights. The most interesting result though is oral health, it seems like more of a unique topic.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** The code shows the connotation of certain words. It doesn't seem as helpful because some words can be used in both positive and negative ways, so it would need more context in order to do this accurately.
