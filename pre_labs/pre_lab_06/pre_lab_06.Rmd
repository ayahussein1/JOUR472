...
**Task** Load the four CSV population estimate files. Do they have the same column structure? **Answer** No, the 2022 column for "Total" is title case instead of uppercase.
...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** There is a greater increase in Democrats than o=in Republicans and there is an increase in overall voters.
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** There are 24 rows, there should be 24 rows because there are 24 counties.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** It has the amount of people of each race in each county in Maryland. They also have an age column for the number of people 18 and over.
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The county with the highest percentage of 18+ Black people is Prince George's County with 60.9%, and the percentage of Democrats in that county is 77.1%.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** The county with the highest 18+ Asian people is Howard County with 19.4%, and the percentage of Democrats in that county is 52.6%. The difference is there is a significantly lower Asian population than Black population in the state.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** The datatype for ZCTA5N is dbl.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N? **Answer** It's now a character.
