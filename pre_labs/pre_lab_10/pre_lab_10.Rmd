...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** There are columns that identify the zipcodes like the zip code and zip name. There are also columns called shape_leng and shape_area that show the dimensions of the zipcode. The geometry column has that creates the boundaries for the map.
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** The output shows the contents of the data frame and all the information for each zip code including the geometry and the name.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output shows a map that has all the zip codes in Maryland on a grid showing the latitude and longitude.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code calls for the tgris package which has geographic information from the census.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output shows the information for every county in the U.S. and whether each variable is a character or double. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code filters the counties data so that it only shows Maryland.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The map has land completly covering the bay and some of the smaller islands off the shore.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code layers the zip code map over the county map. You can see that the counties include the water while the zip codes don't.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code cleans the foreclosure dataset and selects only the zip code and numbers from March 2023, which gives us a smaller dataset that's easier to work with.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code joins the Maryland zipcode geometry dataset with the foreclosure dataset by the zip code.
...
**Task** Run the following code. Describe the output in the space below. **Answer** GGplot is used to create a chloropleth map showing the number of foreclosures by zip code.
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** This code calls the tidyverse, sf, janitor and tidycensus packages.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The code takes the foreclosures dataset and transposes it so that the counties are in rows instead of columns, making it easier to join. The code also calls data from the ACS showing the populations of Maryland counties.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code adds a geometry column to the data set we retrieved from the ACS in the previous task.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code shows the two datasets: foreclosures and population. We can use them to decide what we want to join by.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code helps to clean the ACS dataset so that the county names have the same format as in the foreclosures one. It removes the state name from the NAME column, which has been renamed to "county." It also changes it to title case to account for the lowercase "c" in Baltimore city, because it is an uppercase "c" in the foreclosures dataset.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code joins the two datesets by the county column. Now you have one data frame that includes both the foreclosure and population numbers.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code creates a new column called "rate" where is is calculated how many notices to foreclose were sent per 1000 people in each county. It also arranges the data from the highest rate to the lowest.
...
**Task** Run the following code. Describe the output in the space below. **Answer** Centroids place a point in the middle of each county. Then ggplot is used to create a chloropleth map to show how counties compare to others by rate. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code changes the colors of the map to make them easier to distinguish.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** This code uses logarithmic transformation to distinguish the rates even further. More counties show up in the lowest tier now.
