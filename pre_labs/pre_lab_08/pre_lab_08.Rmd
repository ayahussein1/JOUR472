...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer** It has 3,822 rows and four columns.
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** The first step is to save the results to a new dataframe. The next step is to create a new column pulling the month from the date. Add "label=TRUE" to get the name of the month instead of the number. After that you group by month. Then you get the count of calls for each month. Then arrange it from the month with the most calls, to the one with the least.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A blank space.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The bar chart appears, and it shows all months on the x-axis. The height of the bar represents the count.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The bars are now in order from the month with the shortest bar to the one with the tallest.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It turned the chart so that the bars are horizontal.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It removed the grey background of the chart.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** It changed the color of the background to blue and the lines to white. It also removed the vertical lines on the grid.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It added labels and a title, as well as a caption showing the source.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The table shows how many calls were made on each day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The code creates a line graph that is very difficult to read because of how it's connecting the counts of the calls for each day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** This code breaks the graph down by week, but the labels on the x-axis are all overlapping making them difficult to read.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The code turned the labels on the x-axis to a 45 degree angle making them easier to see.
