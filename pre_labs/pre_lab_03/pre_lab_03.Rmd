...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It does not show any parsing errors and the leading zero in the zip codes is restored.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** Vendor Name and Date are a character variables. Amount is a double variable.
...
**Answer** There are 549 zip codes that have less than five characters. This is because some of the zip codes could be from other countries so they have a different format. Others are using nine digits.
...
**Answer** The fact that some categories that are NAs, indicating that they don't fall under loans or grants. There is also a cell called "L" and another called "Contract."
...
**Answer** The state granted two of the organizations only $60 and $95.98.
...
**Answer** It changes the spaces in the column names into underscores and made the uppercase letters lowercase.
...
**Answer** The first line is creating a new dataframe called "cleaned_md_grants_loans". The second line renames the columns to get rid of the spaces and uppercase letters. The third line renames the column to something easier to identify ("source"). The fourth line makes all the letters in the source, grantee and description columns uppercase to maintain consistency. 
...
**Answer** There are 50 rows that can be duplicates.
...
**Answer** The old dataframe had 17,765 rows and the new dataframe has 17,740 (distinct got rid of 25 rows).
...
**Answer** Now all the zip codes that had nine characters now have five characters in the new column.
...
from the Maryland Tourist Board -> filter()
which zip code ->  group_by(zip5)
most amount of money -> summarize(sum())
arrange


**Answer** The zip code that recieved the most from the Tourism Board is 21202, which is in Baltimore, and from what I can tell it seems to have a lot of 4 and 5-star hotels around the harbor, so it makes sense that a place like that would recieve a lot of funding from the tourism board.
