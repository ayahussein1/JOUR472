...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** Some of the grantees are the same thing but they're spelled differently.
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** It made "Washington County Museum of Fine Arts" all capital. It also made "The Tree House Child Advocacy Center of Montgomery County MD Inc." all capital.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** They changed "WASHINGTON ADVENTIST UNIVERSITY" to title format.I couldn't find any others in the first 30 results.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** From what I can tell there isn't a difference between the previous code and this one. 
...
*When I exported it as a csv file, it actually downloaded as .txt file. I'm not sure why it did that, but I imported the txt file into Google Sheets and re-downloaded it as a .csv file.
