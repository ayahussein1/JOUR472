...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** id is character, seats are doubles (numbers without decimals) 
...
In the space below, write the title of the course from the first row of the "name" column. **Answer** Directed Research
...
**Answer** The output is showing how many classes are in each term in chronological order from the earliest term to the latest one. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The "arrange" function organized the data in descending order by number of classes instead of ascending order by term.
...
**Answer** The data is now grouped by both term and department, so it shows how many classes are in each department for each term.
...
The "sum" function added up the number of seats available in all the classes in each each term.
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** This new codeblock has the mean and median of seats in all the classes for each term. What the top terms have in common is that they are all fall terms.
...
**Answer** The difference bewteen this and the previous code block is that this one is divided into groups based on departments, and it also has the maximum and minimum number of seats a class has within each department.
...
1.  Which classes last the longest (in terms of semesters)?

2.  Which departments do the best job of uploading their syllabi?

3.  Which instructors teach the most classes?
