...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** It uses the first row of data (Maryland totals) as a column, when it's actually the data.
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** The columns are now called x1, x2, x3...etc.
...
**Task** Run the codeblock below. What else do we need to do? **Answer** There are still periods after each county and area name, which would make it more challenging to call up certain rows when doing analysis.
